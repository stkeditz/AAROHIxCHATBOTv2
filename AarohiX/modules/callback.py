...
from AarohiX.database import dil
...
async def cb_handler(_, query: CallbackQuery):
    if query.data == "HELP":
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_dil = dil.find_one({"chat_id": query.message.chat.id})
            if not is_dil:
                await query.edit_message_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ.**")
            if is_dil:
                dil.delete_one({"chat_id": query.message.chat.id})
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_dil = dil.find_one({"chat_id": query.message.chat.id})
            if not is_dil:
                dil.insert_one({"chat_id": query.message.chat.id})
                await query.edit_message_text(
