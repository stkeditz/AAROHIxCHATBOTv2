...
async def executor(client: AarohiX, message: Message):
...
@AarohiX.on_cb("runtime")
async def runtime_func_cq(_, cq):
    runtime = cq.data.split(None, 1)[1]
    await cq.answer(runtime, show_alert=True)


@AarohiX.on_cb("forceclose")
...
async def shellrunner(client: AarohiX, message: Message):
    if len(message.command) < 2:
